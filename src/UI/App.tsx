...
import { Exercise, ExerciseLog } from '../domain';
...
const RoutinesRoute: React.FC = () => {
  const { exercises, muscleGroups, routines, saveRoutine, deleteRoutine, logSession, reorderRoutine, reorderRoutineExercise, updateExerciseNote, updateExerciseLog, deleteExerciseLog, deleteAllLogs, deleteAllLogsExceptLatest, deleteExercise, shareRoutine, importRoutine } = useAppData();
  const { startWorkout } = useWorkoutSession();
  const navigate = useNavigate();
  const [activeRoutineId, setActiveRoutineId] = useState<string | null>(null);

  const handleShareRoutine = (routine: import('../domain').Routine) => {
    const content = shareRoutine(routine.id);
    if (!content) return;
  ...
  const handleStartWorkout = (routine: import('../domain').Routine, dayId: string) => {
