...
function makeDefaultDay(ids: IdGenerator, name?: string): RoutineDay {
  return { id: ids.generate('day'), name: name ?? 'Día 1', exercises: [] };
}
...
export const RoutinesScreen: React.FC<Props> = ({
  ...
  const openCreate = () => {
    setFormName('');
    setFormDays([makeDefaultDay(ids, t.labels.day + ' 1')]);
    setActiveDayIndex(0);
    setFormSearch('');
  ...
  const addDay = () => {
    ...
    setFormDays((prev) => [...prev, makeDefaultDay(ids, `${t.labels.day} ${nextNumber}`)]);
  ...
  const handleSave = () => {
    ...
    onSaveRoutine({ id: editingRoutine?.id ?? ids.generate('routine'), name, days: formDays });
    closeModal();
  };
  ...
  const handleDuplicate = (routine: Routine) => {
    onSaveRoutine({ id: ids.generate('routine'), name: `${routine.name} (2)`, days: routine.days.map((day) => ({ ...day, id: ids.generate('day'), exercises: day.exercises.map((re) => ({ ...re })) })) });
  };
