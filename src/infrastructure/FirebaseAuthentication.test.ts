...
const authentication = new FirebaseAuthentication(localStoragePreferencesRepository);
...
describe('authentication', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
    localStorage.clear();
    localStoragePreferencesRepository.savePrefs({ authMode: null });
  ...
  it('uses signInWithPopup for Google login', async () => {
    ...
    expect(localStoragePreferencesRepository.getPrefs().authMode).toBe('google');
  });
  ...
  it('returns an error when popup is blocked', async () => {
    const { signInWithPopup } = await import('firebase/auth');
    vi.mocked(signInWithPopup).mockRejectedValue({ code: 'auth/popup-blocked', message: 'Popup blocked' });
  ...
  it('returns an error when popup is closed by user', async () => {
    const { signInWithPopup } = await import('firebase/auth');
    vi.mocked(signInWithPopup).mockRejectedValue({ code: 'auth/popup-closed-by-user', message: 'Closed' });
  ...
  it('returns an error when popup request is cancelled', async () => {
    const { signInWithPopup } = await import('firebase/auth');
    vi.mocked(signInWithPopup).mockRejectedValue({ code: 'auth/cancelled-popup-request', message: 'Cancelled' });
  ...
  it('captures redirect result on subscription', async () => {
    ...
    vi.mocked(getRedirectResult).mockResolvedValue({ user: mockUser } as unknown as UserCredential);
