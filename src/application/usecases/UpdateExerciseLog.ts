...
export class UpdateExerciseLog {
  constructor(
  ...
  async execute(id: string, originalDate: string, log: ExerciseLog): Promise<void> {
    const exercise = this.repository.getSnapshot().exercises.find((item) => item.id === id);
    if (!exercise) return;
    const logs = ExerciseLogs.from(exercise.logs).replace(originalDate, log);
    if (!logs) return;
